...
{
  return srslte_symbol_sz_scs(nof_prb, SRSLTE_SCS_15KHZ);
}

int srslte_symbol_sz_scs(uint32_t nof_prb, srslte_scs_t subcarrier_spacing)
{
  if (nof_prb <= 0) {
    return SRSLTE_ERROR;
  }

  if (subcarrier_spacing == SRSLTE_SCS_1KHZ25) {
    if (nof_prb <= 6) {
      return 1536;
    } else if (nof_prb <= 15) {
      return 3072;
  ...
  } else if (subcarrier_spacing == SRSLTE_SCS_7KHZ5) {
    if (nof_prb <= 6) {
      return 256;
    } else if (nof_prb <= 15) {
      return 512;
